...
export const FormDataPersonal = () => {
  ...
  const handleClickResetValueCountry = () => {
    ...
    if(profileStorage !== null && profileStorage !== undefined) {
      const profile = localStorage.getItem("updateInfo");
      const profileData = profile ? JSON.parse(profile) : {};
      const value: number = (profileData.country !== null) ? profileData.country : 1;
          ...
          <div className={style.countryBlock}>
            ...
            {clickCountry && (
              <div className={style.options}>
                { countryList && countryList.map((country: TCountries) => 
                  <div
                    className={selectedCountry !== Number(country.id) ? style.option : style.optionHide}
                    onClick={() => handleOptionClickCountry(Number(country.id))}>
                    {country.name}
                  </div>
                )} 
              </div>
